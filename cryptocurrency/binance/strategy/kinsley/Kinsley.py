...
from cryptocurrency.binance.strategy.kinsley.KinsleyDataProcessor import KinsleyDataProcessor

class Kinsley:
    def __init__(self):
        self.strategy   = "Kinsley"
        ...
        self.kinsleyDataProcessor  = KinsleyDataProcessor(self.strategy, self.instrument, self.product, self.interval)
    ...
    def _start_test_round_backtest(self):
        ...
        for symbol in symbol_list:
            if symbol not in finished_list:
                kinsleyDataProcessor       = KinsleyDataProcessor(self.strategy, self.instrument, self.product, self.interval, symbol)
                backtest_df, finished_path = kinsleyDataProcessor._get_backtest_df_for_backtest_system()
...
def main():
    ...
    # 1st phrase
    kinsley._start_test_round_backtest()
    print("----------------------------------------------------------------------------------------------------")
    exit()
    kinsley._start_first_round_backtest()
    print("----------------------------------------------------------------------------------------------------")
    kinsley._screen_full_backtest_result()
    print("----------------------------------------------------------------------------------------------------")

    """
    # 2nd phrase
    kinsley._manage_full_backtest_result()
    print("----------------------------------------------------------------------------------------------------")
    """

    """
    # final phrase
    kinsley._start_second_round_backtest()
    print("----------------------------------------------------------------------------------------------------")
    kinsley._draw_full_backtest_result_curves()
    print("----------------------------------------------------------------------------------------------------")
    """
