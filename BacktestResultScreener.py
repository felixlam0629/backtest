...
class BacktestResultScreener:
    ...
    def _generate_full_symbol_backtest_report(self):
        ...
        result_list = self.generate_result_list(original_symbol_list, finished_symbol_list, finished_symbol_list_path)
    ...
    def generate_result_list(self, original_symbol_list, finished_symbol_list, finished_symbol_list_path):
        asset      = self.asset
        strategy   = self.strategy
        exchange   = self.exchange
        instrument = self.instrument
        ...
        for symbol in original_symbol_list:
            if symbol in finished_symbol_list:
                ...
                highest_sharpe = symbol_df["strat_sharpe"].iloc[0]
                lowest_sharpe  = symbol_df["strat_sharpe"].iloc[-1]

                num_of_trade = symbol_df["num_of_trade"].iloc[0]
                win_rate     = symbol_df["strat_win_rate"].iloc[0]
                ann_return   = symbol_df["strat_ann_return"].iloc[0]
                mdd          = symbol_df["strat_mdd"].iloc[0]
                calmar       = symbol_df["strat_calmar"].iloc[0]
    ...
    def _generate_result_df(self, result_list):
        ...
        contrary_test_list   = result_list[8]
        rubbish_strat_list   = result_list[9]
        ...
        result_df = pd.concat((original_symbol_df, num_of_trade_df, win_rate_df, ann_return_df, mdd_df, \
                               calmar_df, highest_sharpe_df, lowest_sharpe_df, contrary_test_df, rubbish_strat_df), axis = 1)

        column_name_list  = ["symbol", "num_of_trade", "win_rate", "ann_return", "mdd", \
                             "calmar","highest_sharpe", "lowest_sharpe", "contrary_test", "rubbish_strat"]
