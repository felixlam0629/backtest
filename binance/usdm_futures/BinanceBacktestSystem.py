...
class BinanceBacktestSystem():
    def __init__(self, strategy, instrument, product, interval, symbol,
    ...
    def _run_full_backtest_system(self):
        ...
        for single_df_dict in single_df_list:
            ...
            pool.close()
            print(f"{self.exchange}丨{self.strategy}丨{self.instrument}丨{self.product}丨{self.interval}丨{self.symbol}丨{single_df_name}丨action = finished full backtest")

            full_result_df = self._store_full_result_df(return_list, result_dict, result_key_list, single_df_dict, manager_list)
            print(f"{self.exchange}丨{self.strategy}丨{self.instrument}丨{self.product}丨{self.interval}丨{self.symbol}丨{single_df_name}丨action = exported backtest_report to csv")

        print(f"{self.exchange}丨{self.strategy}丨{self.instrument}丨{self.product}丨{self.interval}丨{self.symbol}丨action = finished backtest")
        print("**************************************************")
    ...
    def _get_single_result_list(self, backtest_ready_list, base_csv_existed, para_combination):
        ...
        print(f"{self.exchange}丨{self.strategy}丨{self.instrument}丨{self.product}丨{self.interval}丨{self.symbol}丨{single_df_name}丨({rolling_window}, {upper_band}, {lower_band})丨action = finished backtest")
