...
class BinanceDataProcessor:
    ...
    def _get_price_path(self):
        ...
        price_path = f"D:/data/{self.asset}/{self.exchange}/{self.instrument}/{self.product}/{price_function}/{self.interval}"
    ...
    def _get_funding_rate_path(self):
        ...
        funding_rate_path = f"D:/data/{self.asset}/{self.exchange}/{self.instrument}/{self.product}/{funding_rate_function}/{self.interval}"
    ...
    def _get_funding_rate_df(self, funding_rate_path):
        ...
        try:
            ...
            message = {"strategy": strategy, "symbol": symbol, "interval": interval, "product": product, "instrument": instrument, "function": function, "exchange": exchange, "asset": asset,  "msg": "no funding_rate data file"}
            ...
            print(strategy, symbol, interval, product, instrument, function, exchange, "response = failed backtest, reason = without funding_rate data file")
            print("**************************************************")
    ...
    """
    def _get_open_interest_path(self): # from binance
        open_interest_function = "open_interest"
        open_interest_interval = "4h"

        open_interest_path = f"D:/data/{self.asset}/{self.exchange}/{self.instrument}/{self.product}/{funding_rate_function}/{open_interest_interval}"

        return open_interest_path
        """

    def _get_open_interest_path(self): # from coinglass
        open_interest_source   = "coinglass"
        open_interest_function = "oi_ohlc"
        open_interest_type     = "perpetual"
        open_interest_interval = "h8"

        open_interest_path = f"D:/data/{self.asset}/{open_interest_source}/{self.instrument}/{open_interest_function}/{open_interest_type}/{open_interest_interval}/{self.exchange}"
    ...
    def _get_open_interest_df(self, open_interest_path):
        ...
        try:
            ...
            message = {"strategy": strategy, "symbol": symbol, "interval": interval, "product": product, "instrument": instrument, "function": function, "exchange": exchange, "asset": asset, "msg": "no open_interest data file"}
            ...
            print(strategy, symbol, interval, product, instrument, function, exchange, "response = failed backtest, reason = without open_interest data file")
            print("**************************************************")
    ...
    def _get_formatted_base_backtest_df(self, open_interest_df, price_fr_df):
        ...
        base_backtest_df = base_backtest_df[["time", "datetime", "open", "fundingRate", "openInterest"]]
    ...
    def _get_finished_path(self):
        finished_path = f"D:/backtest/{self.asset}/{self.exchange}/{self.strategy}/{self.instrument}/{self.product}/{self.interval}"
    ...
    def __create_folders(self):
        ...
        second_folder = f"{first_folder}/{self.instrument}"
        self.__create_folder(second_folder)

        third_folder = f"{second_folder}/{self.product}"
        self.__create_folder(third_folder)

        fourth_folder = f"{third_folder}/{self.interval}"
        self.__create_folder(fourth_folder)
