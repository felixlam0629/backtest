...
class BacktestSystem():
    ...
    def get_backtest_ready_list_without_base_csv(self, para_combination):
        ...
        df["ma"]      = df["basisRate_oi"].rolling(rolling_window).mean()
        df["sd"]      = df["basisRate_oi"].rolling(rolling_window).std()
        df["z_score"] = (df["basisRate_oi"] - df["ma"]) / df["sd"]
...
class DataProcessor:
    ...
    def get_price_fr_basis_df(self):
        ...
        price_fr_basis_df = price_fr_basis_df[["time", "datetime", "open", "fundingRate", "basisRate"]]
    ...
    def get_formatted_backtest_df(self):
        ...
        backtest_df["basisRate_oi"] = backtest_df["basisRate"] / backtest_df["openInterest"]

        backtest_df = backtest_df[["time", "datetime", "open", "fundingRate", "basisRate_oi"]]
