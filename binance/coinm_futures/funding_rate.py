...
class BacktestSystem():
    def __init__(self, df, finished_path, symbol):
        ...
        self.strategy   = "testing_funding_rate"
    ...
    def get_backtest_ready_list_without_base_csv(self, para_combination):
        ...
        df["ma"]      = df["fundingRate"].rolling(rolling_window).mean()
        df["sd"]      = df["fundingRate"].rolling(rolling_window).std()
        df["z_score"] = (df["fundingRate"] - df["ma"]) / df["sd"]
...
class DataProcessor:
    ...
    def get_formatted_backtest_df(self):
        ...
        backtest_df = pd.merge(funding_rate_df, price_df, on = "time", how = "inner")
        backtest_df.rename(columns = {"datetime_x": "datetime"}, inplace = True)
        backtest_df = backtest_df[["time", "datetime", "open", "fundingRate"]]
